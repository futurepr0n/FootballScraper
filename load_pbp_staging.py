...
def setup_staging_table(cursor):
    """Creates or recreates the pbp_csv staging table."""
    print("Setting up 'pbp_csv' staging table...")
    cursor.execute("DROP TABLE IF EXISTS pbp_csv;")
    # Create table with columns that are likely in the CSV, plus our own metadata
    cursor.execute("""
    ...
    """)
    print("Table 'pbp_csv' created successfully.")

def load_pbp_to_staging(directory, conn, cur):
    """Loads play-by-play data from CSV files into the staging table."""
                ...
                for row in reader:
                    try:
                        cur.execute(
                            """
                            INSERT INTO pbp_csv (
                                game_id, play_sequence, Quarter, Time, Down, ToGo, 
                                Location, Play, Playcall
...
if __name__ == '__main__':
    ...
    try:
        ...
        load_pbp_to_staging(pbp_dir, conn, cur)
