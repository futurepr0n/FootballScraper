#!/usr/bin/env python3
"""
Simple CSV Database Loader for NFL Data
Uses existing schedule files to get proper team matchups
"""
...
class SimpleCSVLoader:
    def __init__(self):
        ...
        self.game_matchups = {}  # Cache game ID -> teams mapping
        
    def load_schedule_file(self, season, week, season_type='regular'):
        """Load team matchups from schedule file"""
        ...
        logger.info(f"Loading matchups from {week_file}")
        ...
        with open(week_file, 'r') as f:
            for line in f:
                ...
                # Parse ESPN URL to get game ID and teams
                # Format: https://www.espn.com/nfl/game/_/gameId/401671789
                if 'espn.com/nfl/game/_/gameId/' in line:
                    game_id = line.split('gameId/')[-1].strip()
                    
                    # Try to extract teams from URL or use a simple mapping approach
                    # For now, we'll create placeholder entries and update them later
        ...
        logger.info(f"Loaded {len(self.game_matchups)} game matchups")
    ...
    def get_or_create_game(self, game_id, season, week, season_type):
        """Get or create game record - we'll update team assignments after processing all CSVs"""
        ...
        # Create game with placeholder teams for now
        logger.info(f"Creating game placeholder for game_id: {game_id}")
        
        # Use the first team we can find, we'll fix this later
        self.cursor.execute("SELECT id FROM teams LIMIT 1")
        dummy_team_id = self.cursor.fetchone()['id']
        
        self.cursor.execute("""
            INSERT INTO games (game_id, season, week, season_type, home_team_id, away_team_id, date) 
            VALUES (%s, %s, %s, %s, %s, %s, CURRENT_DATE) 
            RETURNING id
        """, (game_id, season, week, season_type, dummy_team_id, dummy_team_id))
        self.conn.commit()
        return self.cursor.fetchone()['id']
    
    def update_game_teams(self, game_id):
        """Update game with proper team assignments after processing CSVs"""
        # Get all teams that have player stats for this game
        self.cursor.execute("""
        ...
        if len(teams) >= 2:
            # Assign first team as home, second as away
            ...
            logger.info(f"Updated game {game_id}: {teams[1]['abbreviation']} @ {teams[0]['abbreviation']}")
        elif len(teams) == 1:
            logger.warning(f"Game {game_id} only has one team with stats: {teams[0]['abbreviation']}")
    ...
    def update_all_game_teams(self, season, week, season_type='regular'):
        """Update team assignments for all games after CSV processing"""
        logger.info("Updating game team assignments...")
        ...
        for game in games:
            self.update_game_teams(game['id'])
        
        self.conn.commit()
        logger.info("Game team assignments updated")
    
    def load_week_data(self, csv_dir, season, week, season_type='regular'):
        """Load all CSV files for a specific week"""
        # First, load the schedule to get game matchups
        self.load_schedule_file(season, week, season_type)
        ...
        # After processing all CSVs, update game team assignments
        self.update_all_game_teams(season, week, season_type)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Load NFL CSV data with proper team assignments')
